...
In IntelliJ IDEA 2023.3.6, a limitation exists when executing Kotlin programs that are part of a JPMS module. If you
attempt to run a Kotlin program in IntelliJ by right-clicking the file containing the `main` function and
selecting `Run`, you get an error like this:

    Error: Could not find or load main class com.staticnoiselog.ai.MainKot in module jpms.library.deeplearning

This is because IntelliJ does not set the module path correctly. Instead of the JAR archive, it lists
the `build\classes\java\main` directory in the module path.

Here is a workaround:

- In the console window where the error appears, copy the first line. This is the command that IntelliJ has constructed
  to run the Kotlin main function.
- Extract the entire `-p` part (the `--module-path`) and replace the "..\build\classes\java\main" directory with the
  reference to the correct JAR archive, in our example. "..\build\libs\deeplearning.jar".
- Edit the Run/Debug Configuration and paste the entire corrected `-p` part into the "VM options:" field.

Now you should be able to run the Kotlin program, including debugging.

Alternatively, you can disable JPMS by temporarily removing/renaming the `module-info.java` file.

An issue exists for this limitation on the JetBrains'
